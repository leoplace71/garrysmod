...
local function Viewbob(pos, ang, time, intensity)
	...
	eang:Normalize()
	nang:Normalize()
	local vfac = math.Clamp(1 - math.pow(math.abs(oang.p) / 90, 3), 0, 1) * (math.Clamp(ldist / 196, 0, 1) * 0.7 + 0.3) * AngularCompensation * cv_comp:GetFloat()
...
hook.Add("CalcView", "TFA_Viewbob", function(ply, pos, ang, ...)
	...
	tmptbl.origin, tmptbl.angles = Viewbob(tmptbl.origin, tmptbl.angles, ViewBobTime, ViewBobIntensity * cv_intensity:GetFloat())
